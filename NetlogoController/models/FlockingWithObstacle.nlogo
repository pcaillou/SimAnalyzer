...
patches-own[
  obstacle ;; to define the obstacles
]
globals[
  doingMovie;;
]
...
to initObstacles
;;  ask patches with [pxcor = 0 and pycor > -5 and pycor < 5 ] [
  let _size 2
  ask patches with [pxcor > -1 * _size and pxcor < _size and pycor > -1 * _size and pycor < _size ] [
      set obstacle true;
    set pcolor red;
  ]  
  ask turtles with [ isOnObstacle? ][
    let p one-of patches with [ not obstacle]
    let x [ pxcor ] of p
    let y [ pycor ] of p
    setxy x y
  ]
...
to update
  ...
  repeat 5 [ ask turtles [ move ] display ]
...
to move
  ifelse inFrontOfObstacle?
  [ ;ifelse ycor > 0 [ set heading 180] [set heading 360]
;;    face one-of neighbors with [ not obstacle ]
    ...
    set heading heading + (turning * 10)
    fd 0.2
  ]
  [ fd 0.2 
    set turning 0
...
to-report isOnObstacle?
  report [ obstacle ] of patch-here
end


to-report inFrontOfObstacle?
  report count ( patches in-cone 3 20 ) with [ obstacle ] > 0
;;  report [ obstacle ] of patch-ahead 1
end
