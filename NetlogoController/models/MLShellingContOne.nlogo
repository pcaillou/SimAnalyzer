...
to setup
  ...
  set distrib matrix:make-constant nb-category 100000 0
  set total-distrib matrix:make-constant 1 100000 0
  ;; create turtles on random patches.
  crt number
      [ set category random nb-category
        set categoryt (word "c" category)
        set color 5
  ...
  ;;        setxy (((who mod 3) * 10) - 10) ((((who / 2) mod 3) * 10) - 10)
          ...
          if category = 11 [set shape "orbit 4"]
          if category = 12 [set shape "orbit 5"]
          if category = 13 [set shape "orbit 2"]
          ] 
...
;  ask one-of turtles with [ not happy? ]
    [ find-new-spot ]
end
...
to update-distrib
  ...
  set distrib matrix:make-constant nb-category 100000 0
  ask turtles [
    let deb matrix:get distrib category color
    let debt matrix:get total-distrib 0 color
    matrix:set distrib category color (deb + 1)
    matrix:set total-distrib 0 color (debt + 1)
  ]
end
to update-turtles
  ask turtles [
